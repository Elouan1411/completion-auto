...
Ensuite, nous avons ajouté, un programme qui permet de déterminer si la chaîne écrite par l'utilisateur est le préfixe du mot du dictionnaire que nous sommes en train de traiter. Nous l'utiliserons également par la suite pour suggérer les mots les plus cohérent avec la chaîne écrite par l'utilisateur.\\

Ensuite, si la chaîne écrite n'est pas le préfixe du mot et que la distance de Levenshtein est supérieur à 3 entre la chaîne et le mot que l'on est en train de traiter, alors on passe au mot suivant. Sinon, On ajoute le mot à un tas de taille 4. On trie ce tas selon la préfixation, puis selon la distance de Levenshtein, et enfin selon la fréquence. On commence par le tri des préfixes car nous voulons que les suggestions complètent le mot en cours plutôt que de le corriger, comme le fait les suggestions sur un téléphone portable. On observe qu'en utilisant d'abord le tri de la distance en premier, on obtient plutôt une correction possible du mot plutôt qu'une complétion.\\
...
Prenons l'exemple du mot \og{} bonjour \fg{}. Imaginons que nous avons pour l'instant écrit \ voulons écrire le mot \og{} bonj \fg{}, alors les suggestions faites grâce au tri des préfixes seront des mots tels que \og{} bonjour \fg{}, \og{} bonjours \fg{} et \og{} bons \fg{}.
...
Enfin, si la taille du tas est supérieure à 3 alors on retire l'élément avec la plus petite priorité. \\
