...
Nous souhaiterions également remercier chaleureusement Jean-Michel HUFFLEN de nous avoir appris le langage \langage{LaTeX} pour la rédaction de nos rapports ainsi que tous les professeurs que nous avons rencontrés et qui nous ont permis d'acquérir nos connaissances actuelles sur les langages utilisés.\\
...
Voici le visuel final de notre interface en utilisant donc \langage{Python} et sa bibliothèque \langage{Tkinter}. 
...
Une fois que nous avons tout mis en place et que l'application est fonctionnelle, nous avons décider d'ajouter un installeur d'application qui permet de télécharger toutes les dépendances, d'ajouter l'application dans un dossier contenu dans le \langage{PATH}. Le \langage{PATH} est une variable système utilisée par le système d'exploitation pour localiser les fichiers exécutables indispensables depuis une ligne de commande ou une fenêtre de terminal.\\
On crée également un fichier \var{.desktop} pour pouvoir ouvrir l'application depuis le gestionnaire d'application d'\langage{Ubuntu} et appliquer les droits nécessaires aux bons fonctionnements de l'application car certaines commandes nécessitent les droits de l'utilisateur \var{root}, par exemple la lecture des fichiers des périphériques. \\
...
Grâce à toute cette installation, l'application est plus facile d'accès grâce à une icône sur le bureau. 
...
Nous avons eu l'occasion d'utiliser de  nombreux outils pour la communication au sein de notre binôme ainsi que pour la réalisation globale du projet semestriel. Nous avons communiquer sur Discord ainsi que sur WhatsApp et partagé l'avancée des projets sur Git. Nous avons également utilisé \langage{Rust}, \langage{Python} et \langage{Tkinter} pour l'implémentation. Enfin le rapport a été rédigé sur \langage{\LaTeX}.  

\begin{figure}[H]
	\begin{center}
		{\includegraphics[width=\textwidth]{images/outils.png}}
	\end{center}
